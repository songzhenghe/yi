<?php 
include("../x.php");
if(!($_SESSION["auth"]["sound"]&ADD)){
	exit("您无权访问本页！");
}
$step4=$_GET["step4"]?$_GET["step4"]:"1";
if($_POST["Submit_1"]){
	$media_id=$_POST["media_id"];
	if($media_id!=""){
		$_SESSION["media_id"]=$media_id;
		echo "<script>location.href='item_add3.php?step4=2';</script>";
		exit;
	}else{
...
if($_POST["Submit_2"]){
	...
	if($item_id!="" and $next==1){
		...
		echo "<script>alert('数据保存成功！');location.href='item_add3.php?step4=1';</script>";
		exit;
	}else if($title!=""){
		if($_FILES["file"]["name"]!=""){
			$pic=$common_func->fileupload(UF,array(".jpg"),2);
		...
		$query="insert into `".$prefix."item` (`media_id`,`title`,`alt`,`url`,`pic`,`add_man`,`add_time`,`tag`,`order`,`type`,`pathinfo`) values ('$_SESSION[media_id]','$title','$alt','$url','$pic','$add_man','$add_time','$tag','$order','3','2')";
...
<table width="95%" border="1" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<select name="media_id">
	  <option value="">选择音频项目所属分类</option>
	  <?php 
	  	$param="select * from `".$prefix."media` where `tag`='1' and `type`='3'";
		$param=$sql_func->mselect($param);
		$sql_func->choose_select($param,"id","name");
		unset($param);
	  ?>
	</select>
	<span class="red">状态为‘隐藏’的音频分类在此没有显示</span>	</td>
  </tr>
  <tr>
  ...
  <tr>
    <td colspan="2" align="center" class="green">项目信息添加分2步，第一步选择音频分类，第二步上传音频文件设置备注信息！<span class="red">文件格式为MP3</span><br /><a href="item_add-3.php">切换至普通添加</a>
    </td>
  </tr>
</table>
</form>
...
<table width="95%" border="1" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<div id="title2" style="display:none;">
		服务器文件地址：<input name="title" type="text" value="" class="input" /> <span class="green">如：（/fm_files/song.mp3）"/"代表网站根目录，支持http。</span><br />
  ...
  <tr>
    <td colspan="2" align="center" class="green">项目信息添加分2步，第一步选择音频分类，第二步上传音频文件设置备注信息！<span class="red">文件格式为MP3</span></td>
  </tr>
</table>
</form>
<?php 
