...
if($_POST["Submit"]){
	...
	if($media_id!="" and $title!=""){
		...
		if($_FILES["file2"]["name"]!="" and $type==1){
			$title=$common_func->fileupload(UF,array(".mp3"),SYS_UP_FILESIZE/1048576,"file2");
		}else{
		...
		$query="insert into `".$prefix."item` (`media_id`,`title`,`alt`,`url`,`pic`,`add_man`,`add_time`,`tag`,`order`,`type`,`pathinfo`) values ('$media_id','$title','$alt','$url','$pic','$add_man','$add_time','$tag','$order','3','$pathinfo')";
		$sql_func->insert($query,3);
		echo "<script>alert('数据保存成功！');location.href='item_add-3.php';</script>";
		exit;
	}else{
...
<table width="95%" border="1" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	<span class="red">状态为‘隐藏’的音频分类在此没有显示</span>
	</td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<div id="title1">
    	文件：<input name="file2" type="file" class="multi" maxlength="1" accept="mp3" />
		<span class="green">(文件格式为MP3)</span>
	</div>
  ...
  <tr>
    <td colspan="2" align="center"><a href="item_add3.php">切换至高级添加</a></td>
  </tr>
</table>
</form>
</div>
