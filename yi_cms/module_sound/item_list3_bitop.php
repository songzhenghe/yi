	...
	if($_POST["Submit"]){
		...
		if($id!=""){
			if($bit_type=="del"){
				...
				echo "<script>alert('您刚才的批量删除操作已成功执行！');location.href='item_list3.php?media_id=".$media_id."';</script>";
				exit;
			}else if($bit_type=="edit"){
				if(!($_SESSION["auth"]["sound"]&EDI)){
					exit("您无权访问本页！");
				}
				$id=implode(",",$id);
				$tag=$_POST["tag"];
				...
				echo "<script>alert('您刚才的批量更新操作已成功执行！');location.href='item_list3.php?media_id=".$media_id."';</script>";
				exit;
			}
		}else{
			exit;
