...
if (!empty($_FILES)) {
	if($_FILES['Filedata']['size']>0 and $_FILES['Filedata']['size']<=SYS_UP_FILESIZE){
		...
		if(in_array($filetype,array(".swf"))){
			...
			$tag='1';
			$order="1";
			$query="insert into `".$prefix."item` (`media_id`,`title`,`alt`,`url`,`pic`,`add_man`,`add_time`,`tag`,`order`,`type`,`pathinfo`) values ('$media_id','$title','$alt','$url','$pic','$add_man','$add_time','$tag','$order','4','1')";
