<?php 
include("../x.php");
if(!($_SESSION["auth"]["link"]&EDI)){
	exit("您无权访问本页！");
}
$item_id=$sql_func->inject_check(trim($_GET["item_id"]));
if($item_id!="" and $item_id>0){
	$query="select `title`,`alt`,`url`,`pic`,`tag`,`order` from `".$prefix."item` where `id`='$item_id'";
...
if($_POST["Submit"]){
	...
	if($title!="" and $url!=""){
		...
		echo "<script>alert('名称、链接地址不能为空！');window.history.back(-1);</script>";
		exit;
	}
}
?>
...
<table width="95%" border="1" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td align="left">
	<input name="title" type="text" class="input" value="<?php echo $info["title"];?>" />
  ...
  <tr>
    ...
    <td align="left"><input name="url" type="text" class="input" value="<?php echo $info["url"];?>" /> <span class="green">(0-300字符)</span></td>
  </tr>
  <tr>
