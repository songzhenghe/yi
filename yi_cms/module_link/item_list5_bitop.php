	...
	if($_POST["Submit"]){
		...
		if($id!=""){
			if($bit_type=="del"){
				...
				echo "<script>alert('您刚才的批量删除操作已成功执行！');location.href='item_list5.php?media_id=".$media_id."';</script>";
				exit;
				...
				echo "<script>alert('您刚才的批量更新操作已成功执行！');location.href='item_list5.php?media_id=".$media_id."';</script>";
				exit;
			}
		}else{
			exit;
