...
$cid=$sql_func->inject_check(trim($_GET["cid"]));
?>
...
<div id="container">
    ...
    <p class="location">当前位置：新闻列表页</p>
	<?php
		$query="select * from `".$prefix."article` where `category_id`='$cid' and `tag`=1 order by `id` DESC";
    ...
    <div id="content">
        <h1>
		<?php 
			$param="select `name` from `".$prefix."category` where `id`='$cid'";
		...
		?>
                <li><em>[<?php echo $sql_func->id2name($prefix,$info[$i]["add_man"]);?>&nbsp;&nbsp;<?php echo substr($info[$i]["add_time"],0,10);?>]</em><a href="news_content.php?id=<?php echo $info[$i]["id"];?>" title="<?php echo $info[$i]["title"]?>"><?php echo $common_func->substrs($info[$i]["title"],70);?></a></li>
		<?php 
           		}
        ?>
		  <?php
